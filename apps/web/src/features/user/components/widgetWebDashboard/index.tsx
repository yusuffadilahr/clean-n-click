...
import NotificationUser from "../notificationCustom"
    ...
    return (
        <div className="w-full h-full overflow-y-auto bg-white bg-opacity-45 rounded-xl p-4">
            <div className="flex justify-between">
                ...
                <NotificationUser dataOrderNotif={dataOrderNotif} />
            </div>
                ...
                {data?.orders?.map((order, i: number) => (
                    <div key={i} className='flex px-2 justify-between items-center w-full gap-4 border-b pb-3'>
                        ...
                        <div className="flex items-center gap-3">
                            <Link href='/user/dashboard/order?tab=waiting-payment' className='text-blue-500 hover:text-blue-700 text-sm'>
                                Proses
                            </Link>
                        </div>
                    </div>
                ))}
                <Link href='/user/dashboard/order?tab=waiting-payment' className='flex text-sm justify-end text-blue-600 hover:text-blue-800'>
                    Lihat Selengkapnya...
                </Link>
            </div>
        </div>
